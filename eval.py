...
class vgg_loss() :
    def __init__(self) :
        ...
        self.vgg.load_state_dict(torch.load('./model/networks/vgg_normalised.pth'))
...
vgg = vgg_loss()
style = Image.open('./quantitative/style/post.jpg').convert('RGB')
style = transform(style)
# style = transforms.Resize([256,256])(style)
style = torch.unsqueeze(style, dim=0).to('cuda')

for i in range(10) :
    target = Image.open(join('./quantitative/mcc/landscape/post', f'ref_{i+5}.jpg')).convert('RGB')#ref_{i + 5}_fake_B
    target = transform(target)
    target = torch.unsqueeze(target, dim=0).to('cuda')
    # print(target.shape)
    content = Image.open(join('./quantitative/mine/landscape/post', f'ref_{i+5}.png')).convert('RGB')
    content = transform(content)
    content = torch.unsqueeze(content, dim=0).to('cuda')

    # c_loss += vgg.get_C_loss(target, content)
    # s_loss += vgg.get_S_loss(target, style)
    c_loss += vgg.get_C_loss(content, target)
    s_loss += vgg.get_S_loss(style, target)
    
print('c_loss : %4f' %(c_loss / 10.0))
print('s_loss : %4f' %(s_loss / 10.0))
